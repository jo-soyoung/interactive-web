...
(() => {
	...
	const sceneInfo = [
		...
		},
		{
			...
			values: {
				videoImageCount: 960, // 캔버스에 불러올 이미지 갯수
				imageSequence: [0, 959],
	...
	function playAnimation() {
		...
		switch (currentScene) {
			...
			case 2:
				...
				if (scrollRatio <= 0.5) {
					objs.canvas.style.opacity = calcValues(values.canvas_opacity_in)
				} else {
					// out
	...
	window.addEventListener('load', () => {
		...
		window.addEventListener('orientationchange', () => {
			setTimeout(setLayout, 500) // 아이폰 돌리고 0.5초 후에 실행되도록 함
		}) // orientationchange는 모바일에서 가로 세로 바꿀 때 일어나는 이벤트
	})
